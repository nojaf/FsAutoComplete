...
let logger =
  LogProvider.getLoggerByName "TipFormatter"
    ...
    { TagName = "a"
      Formatter =
        ...
        | NonVoidElement (innerText, attributes) ->
          ...
          sprintf "[%s](%s)" innerText href |> Some }
    |> applyFormatter
...
let private buildFormatComment cmt (formatStyle: FormatCommentStyle) (typeDoc: string option) =
  ...
  | FSharpXmlDoc.Text (unprocessed, processed) ->
    try
      ...
      let indentationSize =
        processed |> Array.toList |> findIndentationSize
  ...
  |> List.choose
       ...
       | FSharpToolTipElement.Group items ->
         ...
         let makeTooltip (tipElement: FSharpToolTipElementData<string>) =
           let header =
             tipElement.MainDescription + getRemarks tipElement

           let body =
             buildFormatComment tipElement.XmlDoc FormatCommentStyle.Legacy None
                ...
                (fun i ->
                  let comment =
                    if i.TypeMapping.IsEmpty then
                      ...
                      + "\n\n**Generic Parameters**\n\n"
                      + (i.TypeMapping
                         |> List.map formatGenericParamInfo
                         |> String.concat "\n")
...
let formatDocumentationFromXmlSig
  ...
  =
  let xmlDoc =
    FSharpXmlDoc.XmlDocFileSignature(assembly, xmlSig)

  let comment =
    buildFormatComment xmlDoc FormatCommentStyle.Documentation None
...
let extractSignature (FSharpToolTipText tips) =
  let getSignature (str: string) =
    ...
    if firstLine.StartsWith("type ", StringComparison.Ordinal) then
      let index =
        firstLine.LastIndexOf("=", StringComparison.Ordinal)
  ...
  let firstResult x =
    match x with
    | FSharpToolTipElement.Group gs ->
      List.tryPick
        (fun (t: FSharpToolTipElementData<string>) ->
          if not (String.IsNullOrWhiteSpace t.MainDescription) then
