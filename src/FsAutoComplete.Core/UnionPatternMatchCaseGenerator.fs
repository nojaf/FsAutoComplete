...
let private clauseIsCandidateForCodeGen (cursorPos: Pos) (clause: SynMatchClause) =
  let rec patIsCandidate (pat: SynPat) =
    ...
    | SynPat.Named (innerPat, _, _, _, _) -> patIsCandidate innerPat
    ...
    | SynPat.LongIdent (LongIdentWithDots ([ ident ], []), _, _, _, _, _) when ident.idText = "op_ColonColon" -> false
          ...
          |> Option.orElseWith
               (fun () ->
                 if isExnMatch then
                   ...
                   let currentClause =
                     List.tryFind (posIsInLhsOfClause pos) synMatchClauseList
...
let getWrittenCases (patMatchExpr: PatternMatchExpr) =
  ...
  let rec getCasesInPattern (pat: SynPat) =
    match pat with
    | SynPat.LongIdent (LongIdentWithDots (unionCaseLongIdent, _), _, _, constructorArgs, _, _) ->
...
let shouldGenerateUnionPatternMatchCases (patMatchExpr: PatternMatchExpr) (entity: FSharpEntity) =
  ...
  let writtenCaseCount =
    getWrittenCases patMatchExpr |> Set.count
