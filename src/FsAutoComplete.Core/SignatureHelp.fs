...
let private charAt (lines: ISourceText) (pos: Pos) = (lineText lines pos).[pos.Column - 1]
...
let private getSignatureHelpForFunctionApplication
  ...
  asyncMaybe {
    ...
    let possibleApplicationSymbolLineStr =
      lineText lines possibleApplicationSymbolEnd
    ...
    | :? FSharpMemberOrFunctionOrValue as mfv when isValid mfv ->
      let tooltip =
        tyRes.GetCheckResults.GetToolTipText(
      ...
      | _ ->
        ...
        let definedArgs =
          mfv.CurriedParameterGroups |> Array.ofSeq
        ...
        let methods =
          tyRes.GetCheckResults.GetMethods(
            symbolStart.Line,
            symbolUse.RangeAlternate.End.Column,
...
let private getSignatureHelpForMethod (tyRes: ParseAndCheckResults, caretPos: Pos, lines: ISourceText, triggerChar) =
  asyncMaybe {
    ...
    let isStaticArgTip =
      charAt lines paramLocations.OpenParenLocation = '<'
    ...
    let endPos =
      let last =
        paramLocations.TupleEndLocations |> Array.last
    ...
    let startOfArgs =
      inc lines paramLocations.OpenParenLocation
...
let getSignatureHelpFor (tyRes: ParseAndCheckResults, pos: Pos, lines: ISourceText, triggerChar, possibleSessionKind) =
  asyncResult {
    ...
    let previousNonWhitespaceChar =
      charAt lines previousNonWhitespaceCharPos
