...
let logger =
  LogProvider.getLoggerByName "FsAutoComplete.Sourcelink"

let private sourceLinkGuid =
  System.Guid "CC110556-A091-4D38-9FEC-25AB9A351A6A"

let private embeddedSourceGuid =
  System.Guid "0E8A571B-6926-466E-B4AD-8AB04611F5FE"
...
let private tryGetSourcelinkJson (reader: MetadataReader) =
  ...
  |> Option.map
       (fun bytes ->
         ...
         let blob =
           JsonConvert.DeserializeObject<SourceLinkJson>(byteString)
...
let private isEmbedded (reader: MetadataReader) (handle: DocumentHandle) =
  ...
  let headers =
    reader.GetCustomDebugInformation(entityHandle)
...
let private tryGetUrlWithWildcard
  ...
  | m ->
    let replacement =
      normalizeRepoPath (UMX.tag<RepoPathSegment> m.Groups.[1].Value)
