...
type FSharpCompilerServiceChecker(backgroundServiceEnabled, hasAnalyzers) =
  ...
  let computeAssemblyMap () =
    ...
    | Some root, None, None ->
      sdkRefsLogger.warn (
        Log.setMessage "Couldn't find latest 3.x sdk and runtime versions inside {root}"
        >> Log.addContextDestructured "root" root
      )

      Map.empty
    | Some root, None, _ ->
      sdkRefsLogger.warn (
        Log.setMessage "Couldn't find latest 3.x sdk version inside {root}"
        >> Log.addContextDestructured "root" root
      )
    ...
    | Some root, _, None ->
      sdkRefsLogger.warn (
        Log.setMessage "Couldn't find latest 3.x runtime version inside {root}"
        >> Log.addContextDestructured "root" root
      )
    ...
    | Some dotnetSdkRoot, Some sdkVersion, Some runtimeVersion ->
      ...
      let refs =
        FSIRefs.netCoreRefs dotnetSdkRoot (string sdkVersion) (string runtimeVersion) tfm true

      sdkRefsLogger.info (
        Log.setMessage "Found refs for {sdk} inside {root}"
  ...
  let replaceFrameworkRefs (projOptions: FSharpProjectOptions) =
    let refs, otherOptions =
      projOptions.OtherOptions
      |> Array.partition (fun r -> r.StartsWith "-r:")
    ...
    let fcsAndScriptReferences =
      ...
      |> Array.choose
           (fun r ->
             ...
             if assemblyName.Contains "System.Private" then
               None
             ...
             // .NET 5 SDK (https://github.com/dotnet/runtime/pull/36715)
                     ...
                     || assemblyName.Contains "System.Runtime.WindowsRuntime.UI.Xaml" then
               None
             else
    ...
    { projOptions with
        OtherOptions = Array.append otherOptions mergedRefs }
  ...
  let addLoadedFiles (projectOptions: FSharpProjectOptions) =
    let files =
      Array.append fsiAdditionalFiles projectOptions.SourceFiles
  ...
  member private __.GetNetFxScriptOptions(file: string<LocalPath>, source) =
    async {
      ...
      let allFlags =
        Array.append [| "--targetprofile:mscorlib" |] fsiAdditionalArguments
  ...
  member self.GetProjectOptionsFromScript(file: string<LocalPath>, source, tfm) =
    async {
      ...
      | [] ->
        let refs, otherOpts =
          projOptions.OtherOptions
          |> Array.partition (fun o -> o.StartsWith("-r"))

        logQueueLength
          optsLogger
          (Log.setMessage "Resolved references: {refs}"
           >> Log.addContextDestructured "refs" refs)

        logQueueLength
          optsLogger
          (Log.setMessage "Resolved other options: {otherOpts}"
           >> Log.addContextDestructured "otherOpts" otherOpts)
      | errs ->
        logQueueLength
          optsLogger
  ...
  member __.ParseAndCheckFileInProject(filePath: string<LocalPath>, version, source, options) =
    async {
      let opName =
        sprintf "ParseAndCheckFileInProject - %A" filePath
      ...
      try
        ...
        let parseErrors =
          p.Errors |> Array.map (fun p -> p.Message)
        ...
        | FSharpCheckFileAnswer.Succeeded (c) -> return Ok(ParseAndCheckResults(p, c, entityCache))
      with
  ...
  member __.TryGetRecentCheckResultsForFile(file: string<LocalPath>, options, ?source) =
    let opName =
      sprintf "TryGetRecentCheckResultsForFile - %A" file
  ...
  member __.SetDotnetRoot(directory: DirectoryInfo) =
    if sdkRoot = Some directory then
      ()
    else
