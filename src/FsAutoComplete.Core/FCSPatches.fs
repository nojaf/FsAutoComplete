...
module SyntaxTreeOps =
  ...
  let rec synExprContainsError inpExpr =
    ...
    and walkMatchClauses cl =
      cl
      |> List.exists (fun (SynMatchClause.Clause (_, whenExpr, e, _, _)) -> walkExprOpt whenExpr || walkExpr e)
    ...
    and walkExpr e =
      ...
      | SynExpr.Record (_, origExpr, fs, _) ->
        (match origExpr with
         | Some (e, _) -> walkExpr e
         | None -> false)
        || let flds = fs |> List.choose (fun (_, v, _) -> v) in
           walkExprs flds

      | SynExpr.ObjExpr (_, _, bs, is, _, _) ->
        walkBinds bs
        || walkBinds [ for (SynInterfaceImpl.InterfaceImpl (_, bs, _)) in is do
                         yield! bs ]

      | SynExpr.ForEach (_, _, _, _, e1, e2, _)
      | SynExpr.While (_, e1, e2, _) -> walkExpr e1 || walkExpr e2

      | SynExpr.For (_, _, e1, _, e2, e3, _) -> walkExpr e1 || walkExpr e2 || walkExpr e3

      | SynExpr.MatchLambda (_, _, cl, _, _) -> walkMatchClauses cl

      | SynExpr.Lambda (_, _, _, e, _, _) -> walkExpr e

      | SynExpr.Match (_, e, cl, _) -> walkExpr e || walkMatchClauses cl

      | SynExpr.LetOrUse (_, _, bs, e, _) -> walkBinds bs || walkExpr e

      | SynExpr.TryWith (e, _, cl, _, _, _, _) -> walkExpr e || walkMatchClauses cl

      | SynExpr.TryFinally (e1, e2, _, _, _) -> walkExpr e1 || walkExpr e2
      ...
      | SynExpr.IfThenElse (e1, e2, e3opt, _, _, _, _) -> walkExpr e1 || walkExpr e2 || walkExprOpt e3opt

      | SynExpr.DotIndexedGet (e1, es, _, _) ->
        walkExpr e1
        || walkExprs [ for e in es do
                         yield! e.Exprs ]
      ...
      | SynExpr.MatchBang (_, e, cl, _) -> walkExpr e || walkMatchClauses cl

      | SynExpr.LetOrUseBang (rhs = e1; body = e2; andBangs = es) ->
        walkExpr e1
        || walkExprs [ for (_, _, _, _, e, _) in es do
                         yield e ]
        || walkExpr e2
