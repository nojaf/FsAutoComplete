...
type Commands
  ...
  let segmentRanges (parentRange: Range) (childRanges: Range []) : Range [] =
    ...
    [|
       ...
       // encoding algorithm
       if Pos.posEq firstSegment.Start firstSegment.End then
         ()
       else
         firstSegment
       yield! innerSegments
       if Pos.posEq lastSegment.Start lastSegment.End then
         ()
       else
         lastSegment |]
  ...
  let scrubRanges (highlights: struct (range * _) array) : struct (range * _) array =
    let startToken =
      fun (struct (m: Range, _)) -> m.Start.Line, m.Start.Column
         ...
         (fun (_, highlights) ->
           ...
           let expandParents (struct (parentRange, tokenType) as p) =
             let children =
               ...
               |> Array.except [ p ]
               |> Array.choose
                    (fun (struct (childRange, _)) ->
                      if Range.rangeContainsRange parentRange childRange then
                        Some childRange
                      else
                        None)
  ...
  do
    disposables.Add
    <| backgroundService.MessageReceived.Subscribe
         (fun n ->
           match n with
  ...
  do
    disposables.Add
    <| checker.ScriptTypecheckRequirementsChanged.Subscribe
         (fun () ->
  ...
  do
    disposables.Add
    <| fileChecked.Publish.Subscribe
         (fun (parseAndCheck, file, _) ->
           async {
             try
  ...
  do
    disposables.Add
    <| state.ProjectController.Notifications.Subscribe
         (fun ev ->
           match ev with
           | ProjectResponse.Project (p, isFromCache) ->
             let opts =
               state.ProjectController.GetProjectOptionsForFsproj p.ProjectFileName
        ...
        async {
          ...
          return
            ...
            | ResultOrString.Ok (parseAndCheck) ->
              ...
              let errors =
                Array.append results.Errors parseResult.Errors
    ...
    let filterSymbols symbols =
      ...
      |> Array.where
           (fun (su: FSharpSymbolUse) ->
             ...
             || (su.IsFromType
                 && not (UntypedAstUtils.isTypedBindingAtPosition tyRes.GetAST su.RangeAlternate)))
        ...
        | Some (_, idents) ->
          ...
          | Some entityKind ->
            ...
            | Some sym ->
              ...
              let createEntity =
                ParsedInput.tryFindInsertionContext pos.Line parsedTree maybeUnresolvedIdents TopLevel
              ...
              let openNamespace =
                ...
                |> List.choose
                     (fun (entity, ctx) ->
                       entity.Namespace
                       |> Option.map (fun ns -> ns, entity.Name, ctx))
    ...
    async {
      ...
      | Some (recordEpr, (Some recordDefinition), insertionPos) ->
        if shouldGenerateRecordStub recordEpr recordDefinition then
          ...
          let pos =
            Pos.mkPos insertionPos.InsertionPos.Line insertionPos.InsertionPos.Column
  ...
  member x.Project projectFileName (generateBinlog: bool) =
    async {
      commandsLogger.info (
  ...
  member x.GetRangesAtPosition file positions =
    async {
      ...
      | Ok (opts, text) ->
        ...
        return
          positions
          |> List.map (fun x -> UntypedAstUtils.getRangesAtPosition ast.ParseTree x)
          |> CoreResponse.Res
    ...
    asyncResult {
      ...
      let config =
        ...
        | None ->
          fantomasLogger.warn (
            Log.setMessage
              "No fantomas configuration found for file '{filePath}' or parent directories. Using the default configuration."
            >> Log.addContextDestructured "filePath" file
          )

          Fantomas.FormatConfig.FormatConfig.Default

      let! formatted =
        Fantomas.CodeFormatter.FormatDocumentAsync(
          UMX.untag file,
          Fantomas.SourceOrigin.SourceText text,
          config,
          parsingOptions,
          checker
        )
  ...
  member __.FSharpLiterate(file: string<LocalPath>) =
    async {
      ...
      let html =
        FSharp.Formatting.Literate.Literate.ToHtml parsedFile
  ...
  member __.PipelineHints(tyRes: ParseAndCheckResults) =
    result {
      ...
      let getGenerics line (token: FSharpTokenInfo) =
        ...
        let res =
          tyRes.TryGetToolTip(Pos.fromZ line token.RightColumn) lineStr
      ...
      let hints =
        ...
        |> Array.Parallel.map
             (fun (lastExpressionLine, lastExpressionLineWasPipe, currentIndex, pipeToken) ->
