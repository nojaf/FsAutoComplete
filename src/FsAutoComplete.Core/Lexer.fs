...
module Lexer =
  ...
  let tokenizeLine (args: string []) lineStr =
    ...
    let lineTokenizer =
      sourceTokenizer.CreateLineTokenizer lineStr
    ...
    | SymbolLookupKind.ByLongIdent ->
      ...
      let decreasingTokens =
        match tokensUnderCursor
              |> List.sortBy (fun token -> -token.Token.LeftColumn) with
  ...
  let findLongIdentsAndResidue (col, lineStr: string) =
    let lineStr =
      lineStr.Substring(0, System.Math.Max(0, col))
    ...
    | Some sym ->
      ...
      | text ->
        let res =
          text.Split '.' |> List.ofArray |> List.rev
