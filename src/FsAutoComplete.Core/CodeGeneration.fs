...
type CodeGenerationService(checker: FSharpCompilerServiceChecker, state: State) =
  ...
  member x.ParseFileInProject(fileName) =
    ...
    | ResultOrString.Ok (opts, lines) ->
      try
        checker.TryGetRecentCheckResultsForFile(fileName, opts)
...
module CodeGenerationUtils =
  ...
  let tryFindTokenLPosInRange
    ...
    let range =
      if range.EndLine > document.LineCount then
        ...
        let newEndColumn =
          document.GetLineText1(document.LineCount).Length
    ...
    |> Seq.tryFind
         (fun (line1, tokenInfo) ->
           if range.StartLine = range.EndLine then
             ...
             predicate tokenInfo)
    |> Option.map (fun (line1, tokenInfo) -> tokenInfo, (Pos.fromZ (int line1 - 1) tokenInfo.LeftColumn))
  ...
  let formatArgUsage ctx hasTypeAnnotation (namesWithIndices: Map<string, Set<int>>) (arg: FSharpParameter) =
    ...
    let isOptionalArg =
      hasAttribute<OptionalArgumentAttribute> arg.Attributes
  ...
  let formatArgsUsage ctx hasTypeAnnotation (v: FSharpMemberOrFunctionOrValue) args =
    let isItemIndexer =
      (v.IsInstanceMember && v.DisplayName = "Item")
             ...
             let argsSoFar', namesWithIndices =
               ...
               |> List.fold
                    (fun (acc: string list, allNames) arg ->
                      let name, allNames =
                        formatArgUsage ctx hasTypeAnnotation allNames arg
  ...
  let (|MemberNameAndRange|_|) =
    ...
    | Binding (_access,
               ...
               _attrs,
               _xmldoc,
               SynValData (Some mf, _, _),
               LongIdentPattern (name, range),
               _retTy,
               _expr,
               _bindingRange,
               _seqPoint) when mf.MemberKind = MemberKind.PropertyGet ->
    ...
    | Binding (_access,
               ...
               _expr,
               _bindingRange,
               _seqPoint) when mf.MemberKind = MemberKind.PropertySet ->
  ...
  let formatMembersAt
    ...
    let typeParams =
      Seq.map getTypeParameterName e.GenericParameters
      ...
      |> Seq.groupBy
           (fun (m, insts) ->
             ...
             | TypeOfMember typ ->
               let signature =
                 removeWhitespace (
                   sprintf "%s:%s" m.DisplayName (formatType { ctx with ArgInstantiations = insts } typ)
                 )
  ...
  let rec (|TypeIdent|_|) =
    ...
    | SynType.Var (SynTypar.Typar (s, req, _), _) ->
      match req with
      | NoStaticReq -> Some("'" + s.idText)
      | HeadTypeStaticReq -> Some("^" + s.idText)
