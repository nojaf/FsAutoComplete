...
module DocumentationFormatter =
  ...
  let internal formatLink (name: string) xmlDocSig assemblyName =
    if assemblyName = "-" || xmlDocSig = "_" then
      ...
      let content =
        sprintf """[{ "XmlDocSig": "%s", "AssemblyName": "%s"}]""" xmlDocSig assemblyName
  ...
  let rec formatType (displayContext: FSharpDisplayContext) (typ: FSharpType) : (string * int) list =
    ...
    if typ.IsTupleType || typ.IsStructTupleType then
      ...
      [ for arg in typ.GenericArguments do
          if arg <> typ.GenericArguments.[0] then
            yield separator
      ...
      let newContext =
        displayContext.WithPrefixGenericParameters()
  ...
  let formatGenericParameter includeMemberConstraintTypes displayContext (param: FSharpGenericParameter) =
    ...
    let getConstraint (constrainedBy: FSharpGenericParameterConstraint) =
        ...
        seq {
          ...
          if includeMemberConstraintTypes then
            ...
            if isProperty then
              ...
              yield
                ((c.MemberReturnType |> format displayContext |> fst)
                  .TrimStart())
        }
        |> String.concat ""
      ...
      let delegateConstraint (tc: FSharpGenericParameterDelegateConstraint) =
        sprintf
          "delegate<%s, %s>"
          (tc.DelegateTupledArgumentType
           |> format displayContext
           |> fst)
          (tc.DelegateReturnType
           |> format displayContext
           |> fst)
  ...
  let getFuncSignatureWithIdent displayContext (func: FSharpMemberOrFunctionOrValue) (ident: int) =
    ...
    let functionName =
      let name =
        if func.IsConstructor then
          ...
          FSharpKeywords.QuoteIdentifierIfNeeded func.LogicalName
        else
          FSharpKeywords.QuoteIdentifierIfNeeded func.DisplayName
    ...
    let formatName indent padding (parameter: FSharpParameter) =
      let name =
        Option.defaultValue parameter.DisplayName parameter.Name
    ...
    | [ [ p ] ] when
      maybeGetter
      && formatParameter displayContext p |> fst = "unit"
      -> //Member or property with only getter
      modifiers ++ functionName + ":" ++ retType
    | many ->
      ...
      let allParamsLengths =
        many
        |> List.map (
          List.map (fun p -> (formatParameter displayContext p |> snd))
          >> List.sum
        )
      ...
      let formatParameterPadded length p =
        let paddedParam =
          formatName indent padLength p
          ++ (parameterTypeWithPadding p length)
      ...
      let typeArguments =
        ...
        + "\n"
        + indent
        + (String.replicate (max (padLength - 1) 0) " ")
        + "->"
        ++ retType
        ++ retTypeConstraint
      ...
      | _ex ->
        try
          if func.FullType.GenericArguments.Count > 0 then
            let lastArg =
              func.FullType.GenericArguments |> Seq.last
          ...
          |> List.map
               (fun (paramTypes) ->
                 ...
                 |> List.map
                      (fun p ->
                        formatName p + ":"
                        ++ fst (formatParameter displayContext p))
                 |> String.concat (" * "))
          |> String.concat ("-> ")
  ...
  let getValSignature displayContext (v: FSharpMemberOrFunctionOrValue) =
    let retType =
      v.FullType |> format displayContext |> fst
  ...
  let getFieldSignature displayContext (field: FSharpField) =
    let retType =
      field.FieldType |> format displayContext |> fst
  ...
  let getAttributeSignature displayContext (attr: FSharpAttribute) =
    let name =
      formatLink attr.AttributeType.DisplayName attr.AttributeType.XmlDocSig attr.AttributeType.Assembly.SimpleName
  ...
  let getEntitySignature displayContext (fse: FSharpEntity) =
    ...
    let delegateTip () =
      ...
      let invokerSig =
        getFuncSignatureWithIdent displayContext invoker 6
    ...
    let typeTip () =
      ...
      let types =
        ...
        |> Seq.map
             (fun ne ->
               (typeName ne)
               ++ fst (formatLink ne.DisplayName ne.XmlDocSig ne.Assembly.SimpleName))
        |> Seq.toArray
    ...
    let typeDisplay =
      let name =
        let normalisedName =
          FSharpKeywords.QuoteIdentifierIfNeeded fse.DisplayName
  ...
  let footerForType (entity: FSharpSymbolUse) =
    try
      ...
      | SymbolUse.MemberFunctionOrValue m ->
        ...
        | Some e ->
          let link =
            fst (formatLink e.DisplayName e.XmlDocSig e.Assembly.SimpleName)
  ...
  let getTooltipDetailsFromSymbolUse (symbol: FSharpSymbolUse) =
    ...
    | SymbolUse.TypeAbbreviation (fse) ->
      try
        ...
        | FSharpEntity (ent, _, _) ->
          let signature =
            getEntitySignature symbol.DisplayContext ent
    ...
    | SymbolUse.Entity (fse, _) ->
      try
        let signature =
          getEntitySignature symbol.DisplayContext fse
    ...
    | SymbolUse.Constructor func ->
      ...
      | Some ent when ent.IsValueType || ent.IsEnum ->
        //ValueTypes
        let signature =
          getFuncSignature symbol.DisplayContext func
    ...
    | SymbolUse.Property prop ->
      let signature =
        getFuncSignature symbol.DisplayContext prop
    ...
    | SymbolUse.Val func ->
      //val name : Type
      let signature =
        getValSignature symbol.DisplayContext func

      Some((signature, emptyTypeTip), footerForType symbol, cn)

    | SymbolUse.Field fsf ->
      let signature =
        getFieldSignature symbol.DisplayContext fsf

      Some((signature, emptyTypeTip), footerForType symbol, cn)

    | SymbolUse.UnionCase uc ->
      let signature =
        getUnioncaseSignature symbol.DisplayContext uc

      Some((signature, emptyTypeTip), footerForType symbol, cn)

    | SymbolUse.ActivePatternCase apc ->
      let signature =
        getAPCaseSignature symbol.DisplayContext apc

      Some((signature, emptyTypeTip), footerForType symbol, cn)

    | SymbolUse.ActivePattern ap ->
      let signature =
        getFuncSignature symbol.DisplayContext ap
  ...
  let getTooltipDetailsFromSymbol (symbol: FSharpSymbol) =
    ...
    | EntityFromSymbol (fse, _) ->
      try
        let signature =
          getEntitySignature lastDisplayContext fse
    ...
    | UnionCase uc ->
      let signature =
        getUnioncaseSignature lastDisplayContext uc

      Some((signature, emptyTypeTip), footerForType' symbol, cn)

    | ActivePatternCase apc ->
      let signature =
        getAPCaseSignature lastDisplayContext apc
