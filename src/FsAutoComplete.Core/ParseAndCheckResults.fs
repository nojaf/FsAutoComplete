...
type ParseAndCheckResults
  ...
  let logger =
    LogProvider.getLoggerByName "ParseAndCheckResults"
    ...
    async {
      let tryGetFullPath fileName =
        try
          ...
          let basePath =
            Path.GetDirectoryName(UMX.untag __.FileName)
      ...
      | Some (col, identIsland) ->
        ...
        | Some sym ->
          ...
          let rec tryGetSource (ty: FSharpEntity option) =
            async {
              match ty
                    |> Option.map (fun ty -> ty, ty.DeclarationLocation) with
             ...
             (fun e ->
               ...
               | FSharpEntity (ent, _, _) ->
                 match ent.MembersFunctionsAndValues
                       |> Seq.tryFind (fun f -> f.XmlDocSig = xmlSig) with
    ...
    | Some (colu, identIsland) ->
      ...
      | Some symboluse ->
        ...
        | :? FSharpField as symbol ->
          let typ =
            symbol.FieldType.Format symboluse.DisplayContext
  ...
  member __.TryGetF1Help (pos: Pos) (lineStr: LineStr) =
    ...
    | Some (colu, identIsland) ->
      ...
      let help =
        checkResults.GetF1Keyword(pos.Line, colu, lineStr, identIsland)

      match help with
      | None -> ResultOrString.Error "No symbol information found"
      | Some hlp -> Ok hlp

  member __.TryGetCompletions (pos: Pos) (lineStr: LineStr) filter (getAllSymbols: unit -> AssemblySymbol list) =
    async {
      try
        let longName =
          ...
          let sortedDecls =
            ...
            |> Array.sortWith
                 (fun x y ->
                   ...
                   let mutable n =
                     (not x.IsResolved).CompareTo(not y.IsResolved)
  ...
  member __.GetSemanticClassification =
