                 ...
                 member _.VisitBinding(defaultTraverse, binding) =
                   ...
                   | SynBinding.Binding (_, SynBindingKind.NormalBinding, _, _, _, _, _, pat, _, _, _, _) as binding ->
                     if posBetween binding.RangeOfHeadPat pos then
                       ...
                       match pat with
                       | SynPat.LongIdent (LongIdentWithDots ([ id ], _), _, _, _, _, _) when
                         id.idText.StartsWith("op_")
                         ->
  ...
  Run.ifDiagnosticByCode
    ...
    (fun diagnostic codeActionParams ->
      asyncResult {
        ...
        let! rangeOfBinding =
          tyres.GetParseResults.TryRangeOfBindingWithHeadPatternWithPos(fcsRange.Start)
