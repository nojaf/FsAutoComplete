...
type FSharpLspServer(backgroundServiceEnabled: bool, state: State, lspClient: FSharpLspClient) =
  ...
  let diagnosticCollections =
    System.Collections.Concurrent.ConcurrentDictionary<DocumentUri * string, Diagnostic []>()
  ...
  let handleCommandEvents (n: NotificationEvent) =
    try
      ...
      | NotificationEvent.ParseError (errors, file) ->
        ...
        diagnosticCollections.AddOrUpdate((uri, "F# Compiler"), [||], (fun _ _ -> [||]))
        |> ignore

        let diags =
          errors |> Array.map (fcsErrorToDiagnostic)

        diagnosticCollections.AddOrUpdate((uri, "F# Compiler"), diags, (fun _ _ -> diags))
        |> ignore
      ...
      | NotificationEvent.UnusedOpens (file, opens) ->
        ...
        diagnosticCollections.AddOrUpdate((uri, "F# Unused opens"), [||], (fun _ _ -> [||]))
        |> ignore
        ...
        diagnosticCollections.AddOrUpdate((uri, "F# Unused opens"), diags, (fun _ _ -> diags))
        |> ignore
      ...
      | NotificationEvent.SimplifyNames (file, decls) ->
        ...
        diagnosticCollections.AddOrUpdate((uri, "F# simplify names"), [||], (fun _ _ -> [||]))
        |> ignore
        ...
        diagnosticCollections.AddOrUpdate((uri, "F# simplify names"), diags, (fun _ _ -> diags))
        |> ignore
      ...
      | NotificationEvent.AnalyzerMessage (messages, file) ->
        ...
        diagnosticCollections.AddOrUpdate((uri, "F# Analyzers"), [||], (fun _ _ -> [||]))
        |> ignore
        ...
        | messages ->
          ...
          diagnosticCollections.AddOrUpdate((uri, "F# Analyzers"), diag, (fun _ _ -> diag))
          |> ignore
          ...
          [| Run.ifEnabled (fun _ -> config.UnusedOpensAnalyzer) UnusedOpens.fix
             ...
             ExternalSystemDiagnostics.analyzers
               (fun fileUri ->
                 ...
                 | (false, _) -> None)
             Run.ifEnabled (fun _ -> config.InterfaceStubGeneration) (GenerateInterfaceStub.fix tryGetParseResultsForFile commands.GetInterfaceStub getInterfaceStubReplacements)
             Run.ifEnabled (fun _ -> config.RecordStubGeneration) (GenerateRecordStub.fix tryGetParseResultsForFile commands.GetRecordStub getRecordStubReplacements)
             Run.ifEnabled (fun _ -> config.AbstractClassStubGeneration) (GenerateAbstractClassStub.fix tryGetParseResultsForFile commands.GetAbstractClassStub getAbstractClassStubReplacements)
        ...
        | CoreResponse.Res (HelpText.Full (name, tip, additionalEdit)) ->
          ...
          let edits, label =
            ...
            | Some { Namespace = ns; Position = fcsPos } ->
              Some [| { TextEdit.NewText = $"open {ns}"
                        TextEdit.Range = fcsPosToProtocolRange fcsPos } |],
              $"{ci.Label} (open {ns})"
  ...
  override x.TextDocumentSignatureHelp(sigHelpParams: SignatureHelpParams) =
    ...
    |> x.positionHandlerWithLatest
         (fun sigHelpParams fcsPos tyRes lineStr lines ->
           asyncResult {
             let charAtCaret =
               sigHelpParams.Context
               |> Option.bind (fun c -> c.TriggerCharacter)

             match! commands.MethodsForSignatureHelp(tyRes, fcsPos, lines, charAtCaret, sigHelpKind)
                    |> AsyncResult.ofStringErr with
             | None ->
               sigHelpKind <- None
               return! success None
             | Some sigHelp ->
         ...
         (fun p pos tyRes lineStr lines ->
           asyncResult {
             match! commands.SymbolUseProject tyRes pos lineStr
                    |> AsyncResult.ofCoreResponse with
           ...
           asyncResult {
             ...
             let filtered =
               if p.Context.IncludeDeclaration then
                 ...
                 |> Array.filter
                      (fun r ->
                        FSharp.Compiler.Text.Range.rangeContainsPos r pos
                        |> not)

             return
               filtered
               |> Array.map fcsRangeToLspLocation
               |> Some
           })
          ...
          if typ = "signature" then
            ...
            let res =
              ...
              | CoreResponse.Res (LocationResponse.Use (sym, uses)) ->
                ...
                let cmd =
                  { Title = formatted
                    Command = Some "fsharp.showReferences"
                    Arguments = Some args }
  ...
  member private x.handleSemanticTokens(getTokens: Async<CoreResponse<option<(struct (FcsRange * SemanticClassificationType)) array>>>) : AsyncLspResult<SemanticTokens option> =
    asyncResult {
  ...
  member __.FSharpLiterate(p: FSharpLiterateRequest) =
    async {
      logger.info (
        Log.setMessage "FSharpLiterate Request: {parms}"
        >> Log.addContextDestructured "parms" p
      )
