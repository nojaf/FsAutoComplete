...
module Conversions =
  ...
  let getSymbolInformations
    ...
    (topLevel: FSharpNavigationTopLevelDeclaration)
    ...
    let inner (container: string option) (decl: FSharpNavigationDeclarationItem) : SymbolInformation option =
      ...
      let kind =
        defaultArg (glyphToSymbolKind decl.Glyph) SymbolKind.Module
...
module internal GlyphConversions =
  let internal glyphToKindGenerator<'kind when 'kind: equality>
    ...
    let unionCases =
      FSharpType.GetUnionCases(typeof<FSharpGlyph>)

    let cache =
      Dictionary<FSharpGlyph, 'kind option>(unionCases.Length)

    for info in unionCases do
      let glyph =
        FSharpValue.MakeUnion(info, [||]) :?> FSharpGlyph
...
let encodeSemanticHighlightRanges
  (rangesAndHighlights: (struct (LanguageServerProtocol.Types.Range * ClassificationUtils.SemanticTokenTypes * ClassificationUtils.SemanticTokenModifier list)) array)
  =
  let fileStart =
    { Start = { Line = 0; Character = 0 }
      End = { Line = 0; Character = 0 } }
  ...
  let computeLine
    (prev: LanguageServerProtocol.Types.Range)
    ((range, ty, mods): struct (LanguageServerProtocol.Types.Range * ClassificationUtils.SemanticTokenTypes * ClassificationUtils.SemanticTokenModifier list))
    : uint32 [] =
    let lineDelta =
  ...
  | _ ->
    let finalArray =
      Array.zeroCreate (rangesAndHighlights.Length * 5) // 5 elements per entry
