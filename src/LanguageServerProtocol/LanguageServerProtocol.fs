...
module LspJsonConverters =
        ...
        (fun t ->
          if not (FSharpType.IsUnion t) then
            ...
            t
              .BaseType
              .GetCustomAttributes(
                typedefof<ErasedUnionAttribute>,
                false
              )
              .Length > 0)
  ...
  type SingleCaseUnionConverter() =
    ...
    override _.ReadJson(reader: Newtonsoft.Json.JsonReader, t, _existingValue, serializer) =
      ...
      match FSharpType.GetUnionCases(t)
            |> Array.tryFind (fun c -> c.Name.Equals(caseName, StringComparison.OrdinalIgnoreCase)) with
  ...
  type U2BoolObjectConverter() =
    ...
    override _.ReadJson(reader, t, _existingValue, serializer) =
      ...
      | JsonToken.StartObject ->
        // Second side
        let value =
          serializer.Deserialize(reader, (t.GetGenericArguments().[1]))

        FSharpValue.MakeUnion(cases.[1], [| value |])
      | _ -> failwithf $"Unrecognized json TokenType '%s{string reader.TokenType}' when reading value of type '{t.FullName}'"
...
module JsonRpc =
  ...
  let getMessageType messageTest =
    ...
    | { Version = "2.0"
        Id = Some _
        Method = Some _ } -> MessageType.Request
    | { Version = "2.0"
        Id = Some _
        Method = None } -> MessageType.Response
    | { Version = "2.0"
        Id = None
        Method = Some _ } -> MessageType.Notification
    | _ -> MessageType.Error
  ...
  type Error =
    ...
    static member ParseError =
      Error.Create(ErrorCodes.parseError, "Parse error")

    static member InvalidRequest =
      Error.Create(ErrorCodes.invalidRequest, "Invalid Request")

    static member MethodNotFound =
      Error.Create(ErrorCodes.methodNotFound, "Method not found")

    static member InvalidParams =
      Error.Create(ErrorCodes.invalidParams, "Invalid params")

    static member InternalError =
      Error.Create(ErrorCodes.internalError, "Internal error")

    static member InternalErrorMessage message =
      Error.Create(ErrorCodes.internalError, message)
...
let private notImplemented<'t> =
  async.Return LspResult.notImplemented<'t>
...
module Server =
    ...
    let tokenRun (lspServer: 'server) (paramsToken: JToken option) : AsyncLspResult<JToken option> =
      try
        let p =
          ...
          | Some paramsToken ->
            let typedParams =
              paramsToken.ToObject<'param>(jsonSerializer)
        ...
        | None -> async.Return(Result.Error(Error.Create(ErrorCodes.invalidRequest, "No params found")))
      with
      | :? JsonException as ex -> async.Return(Result.Error(Error.Create(ErrorCodes.parseError, ex.ToString())))
  ...
  let handleRequest<'server when 'server :> LspServer> (requestHandlings: Map<string, RequestHandling<'server>>) (request: JsonRpc.Request) (lspServer: 'server) : Async<JsonRpc.Response option> =
    async {
      ...
      | Some handling ->
        ...
        with
        | ex -> methodCallResult <- Result.Error(Error.Create(ErrorCodes.internalError, ex.ToString()))
      | None -> ()

      match methodCallResult with
      | Result.Ok ok -> return Some(JsonRpc.Response.Success(request.Id, ok))
      | Result.Error err -> return Some(JsonRpc.Response.Failure(request.Id, err))
    }
  ...
  let handleNotification<'server when 'server :> LspServer> (requestHandlings: Map<string, RequestHandling<'server>>) (notification: JsonRpc.Notification) (lspServer: 'server) : Async<Result<unit, _>> =
    async {
      ...
      match requestHandlings
            |> Map.tryFind notification.Method with
      | Some handling ->
        try
  ...
  let start<'a, 'b when 'a :> LspClient and 'b :> LspServer> (requestHandlings: Map<string, RequestHandling<'b>>) (input: Stream) (output: Stream) (clientCreator: (ClientNotificationSender * ClientRequestSender) -> 'a) (serverCreator: 'a -> 'b) =
    ...
    let responseAgent =
      MailboxProcessor<ResponseMailboxMsg>.Start
        (fun agent ->
          let rec loop state =
            async {
              ...
              match msg with
              | Request (rid, reply) -> return! loop (state |> Map.add rid reply)
              | Response (Some rid, value) ->
                let result = state |> Map.tryFind rid

                match result with
                | Some (reply) -> reply.Reply(value.Result)
                | None ->
                  logger.warn (
                    Log.setMessage "ResponseAgent - Unexpected response (id {rid}) {response}"
                    >> Log.addContextDestructured "response" value
                    >> Log.addContext "rid" rid
                  )

                return! loop (state |> Map.remove rid)
              | Response (None, response) ->
                logger.error (
                  Log.setMessage "ResponseAgent - Client reports we sent a corrupt request id, error: {error}"
                  >> Log.addContextDestructured "error" (response.Error)
                )
    ...
    let sendServerNotification (rpcMethod: string) (notificationObj: obj) : AsyncLspResult<unit> =
      let serializedNotification =
        JToken.FromObject(notificationObj, jsonSerializer)

      let notification =
        JsonRpc.Notification.Create(rpcMethod, Some serializedNotification)
    ...
    let sendServerRequest (rpcMethod: string) (requestObj: obj) : AsyncLspResult<'response> =
      async {
        ...
        let req =
          JsonRpc.Request.Create(getNextRequestId (), rpcMethod, serializedRequest)

        let reqString =
          JsonConvert.SerializeObject(req, jsonSettings)
        ...
        try
          ...
          | Some responseToken ->
            let typedResponse =
              responseToken.ToObject<'response>(jsonSerializer)
          ...
          | None ->
            if typeof<'response> = typeof<unit> then
              ...
              logger.error (
                Log.setMessage "ResponseHandling - Invalid response type for response {response}, expected {expectedType}, got unit"
                >> Log.addContextDestructured "response" response
                >> Log.addContext "expectedType" (typeof<'response>).Name
              )
        ...
        | :? JsonException as ex ->
          logger.error (
            Log.setMessage "ResponseHandling - Parsing failed for response {response}"
            >> Log.addContextDestructured "response" response
            >> Log.addExn ex
          )
    ...
    let handleClientMessage (messageString: string) : MessageHandlingResult =
      let messageTypeTest =
        JsonConvert.DeserializeObject<JsonRpc.MessageTypeTest>(messageString, jsonSettings)

      match getMessageType messageTypeTest with
      | MessageType.Response ->
        let response =
          JsonConvert.DeserializeObject<JsonRpc.Response>(messageString, jsonSettings)

        responseAgent.Post(Response(response.Id, response))
        MessageHandlingResult.Normal
      | MessageType.Notification ->
        let notification =
          JsonConvert.DeserializeObject<JsonRpc.Notification>(messageString, jsonSettings)
        ...
        async {
          ...
          | Result.Error error ->
            logger.error (
              Log.setMessage "HandleClientMessage - Error {error} when handling notification {notification}"
      ...
      | MessageType.Request ->
        let request =
          JsonConvert.DeserializeObject<JsonRpc.Request>(messageString, jsonSettings)
      ...
      | MessageType.Error ->
        logger.error (
          Log.setMessage "HandleClientMessage - Message had invalid jsonrpc version: {messageTypeTest}"
  ...
  type Client(exec: string, args: string, notificationHandlings: Map<string, NotificationHandler>) =
    ...
    let handleRequest (request: JsonRpc.Request) =
      async {
        ...
        match notificationHandlings
              |> Map.tryFind request.Method with
        | Some handling ->
          try
    ...
    let messageHanlder str =
      let messageTypeTest =
        JsonConvert.DeserializeObject<JsonRpc.MessageTypeTest>(str, jsonSettings)

      match getMessageType messageTypeTest with
      | MessageType.Notification ->
        let notification =
          JsonConvert.DeserializeObject<JsonRpc.Notification>(str, jsonSettings)
      ...
      | MessageType.Request ->
        let request =
          JsonConvert.DeserializeObject<JsonRpc.Request>(str, jsonSettings)
    ...
    member __.SendNotification (rpcMethod: string) (requestObj: obj) =
      ...
      let notification =
        JsonRpc.Notification.Create(rpcMethod, Some serializedResponse)
